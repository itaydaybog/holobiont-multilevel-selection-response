...
import numpy as np
import MLS_static_fast_new as mlssf
...
x_axis_parameter_ratio = 2  # We tested [-2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2]
y_axis_parameter_ratio = 4  # We tested [-2, -1.25, -0.5, 0.25, 1, 1.75, 2.5, 3.25, 4]
...
figureName = 'figure3_step_%.2f_%.2f.pdf' % (x_axis_parameter_ratio, y_axis_parameter_ratio)
dataName = 'data_Figure3_step_%.2f_%.2f.npz' % (x_axis_parameter_ratio, y_axis_parameter_ratio)
...
desiredTauV = np.logspace(*(x_axis_parameter_ratio, x_axis_parameter_ratio), nStep) * tau_H
desiredTauH = np.logspace(*(y_axis_parameter_ratio, y_axis_parameter_ratio), nStep)
...
def run_model():
    ...
    results = Parallel(n_jobs=nJobs, verbose=9, timeout=1.E6)(
